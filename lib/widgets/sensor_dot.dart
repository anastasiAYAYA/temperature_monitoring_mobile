...
class SensorDot extends StatelessWidget {
  // класс для отображения точки датчика
  const SensorDot({
    super.key,
    required this.state,
    this.sensor,
  ...
  Widget build(BuildContext context) {
    final color = _stateColor(context, state); // цвет точки
  ...
  Color _stateColor(BuildContext context, SensorState s) => switch (s) {
    SensorState.normal => AppColors.of(context).green, // зеленый цвет
    SensorState.warning => AppColors.of(context).orange, // оранжевый цвет
    SensorState.critical => AppColors.of(context).red, // красный цвет
  };
}

class _SensorCard extends StatelessWidget {
  // класс для отображения карточки датчика
  const _SensorCard({
  ...
  @override
  Widget build(BuildContext context) {
    // функция для построения карточки датчика
    ...
    final shortName = name.length > 8
        ? '${name.substring(0, 7)}…'
        : name; // сокращенное название датчика
    ...
    return Container(
      // контейнер для карточки датчика
      constraints: const BoxConstraints(
        minWidth: 42,
        maxWidth: 62,
      ), // ограничения для карточки датчика
      padding: const EdgeInsets.symmetric(
        horizontal: 4,
        vertical: 3,
      ), // отступы для карточки датчика
      decoration: BoxDecoration(
        color: bgColor, // цвет карточки датчика (зависит от темы)
        borderRadius: BorderRadius.circular(
          5,
        ), // закругление углов карточки датчика
        border: Border.all(
          color: borderColor,
          width: 1.0,
        ), // граница карточки датчика
        boxShadow: [
          BoxShadow(
