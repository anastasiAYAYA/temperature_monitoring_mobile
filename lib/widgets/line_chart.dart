...
class LineChartWidget extends StatefulWidget { // класс для построения линейного графика
...
class _LineChartWidgetState extends State<LineChartWidget> { // состояние для линейного графика
  ...
  static const double _padRight  = 64.0; // отступ справа (место для меток порогов)

  int _indexFromDx(double dx, double chartW) { // функция для получения индекса из координаты x
    if (widget.points.length <= 1) return 0; // если точек меньше 1, то возвращаем 0
  ...
  void _onMove(Offset local, double totalW) { // функция для перемещения hover
    final chartW = totalW - _padLeft - _padRight; // ширина графика
    final idx = _indexFromDx(local.dx, chartW); // индекс
    if (_hoverIndex != idx) setState(() => _hoverIndex = idx); // установка индекса
  }

  void _onExit() => setState(() => _hoverIndex = null); // функция для выхода из hover

  @override
  Widget build(BuildContext context) { // функция для построения виджета
    if (widget.points.isEmpty) { // если точек нет
      return const SizedBox( // коробка
        height: 160, // высота
        child: Center( // центр
          child: Text('Нет данных', style: TextStyle(color: _kTextDim, fontSize: 13)), // текст
        ),
      ); // коробка
    }

    final lineColor = widget.color ?? Theme.of(context).colorScheme.primary; // цвет линии
    final isDark = Theme.of(context).brightness == Brightness.dark; // тёмная ли тема

    return LayoutBuilder(builder: (context, constraints) { // функция для построения виджета
      final totalW = constraints.maxWidth; // ширина
      return MouseRegion( // мышь
        onHover: (e) => _onMove(e.localPosition, totalW), // наведение
        onExit:  (_)  => _onExit(), // выход
        child: GestureDetector( // детектор жестов
        onTapDown:   (d) => _onMove(d.localPosition, totalW), // нажатие
        onTapUp:     (_) => _onExit(), // отпускание
        onPanUpdate: (d) => _onMove(d.localPosition, totalW), // обновление пана
...
class _ChartPainter extends CustomPainter { // класс для построения графика
  _ChartPainter({
    required this.points, // список точек
  ...
  static TextPainter _tp(String text, {double size = 10, Color color = _kTextDim, FontWeight fw = FontWeight.w400}) { // функция для построения текста
    final tp = TextPainter(
      text: TextSpan(text: text, style: TextStyle(color: color, fontSize: size, fontWeight: fw)), // текст
      textDirection: TextDirection.ltr, // текст справа налево
    )..layout(maxWidth: 80); // размер текста
    return tp; // текст
  }

  String _fmtX(int index) { // функция для форматирования оси X
    if (timestamps == null || timestamps!.isEmpty) return ''; // если временных меток нет, то возвращаем пустую строку
    final ts = timestamps![index.clamp(0, timestamps!.length - 1)].toLocal(); // время
    switch (period) { // период
      case 'День':
  ...
  String _fmtHover(int index) { // функция для форматирования hover
  ...
  @override
  void paint(Canvas canvas, Size size) { // функция для построения графика
    if (points.isEmpty) return; // если точек нет, то возвращаем
    try { _doPaint(canvas, size); } // построение графика
    catch (e) { debugPrint('[LineChart] $e'); } // ошибка
  }

  void _doPaint(Canvas canvas, Size size) { // функция для построения графика
    final chartW = size.width  - padLeft - padRight; // ширина графика
    final chartH = size.height - padTop  - padBottom; // высота графика

    // ── Диапазон Y ────────────────────────────────────────────────────────────
    final allY = [ // список всех значений
      ...points,
      if (warningMin != null) warningMin!, // минимальное значение для предупреждения
      if (warningMax != null) warningMax!, // максимальное значение для предупреждения
    ...
    final rawMax = allY.reduce((a, b) => a > b ? a : b); // максимальное значение
    ...
    final dRange = (dMax - dMin).abs() < 0.0001 ? 1.0 : dMax - dMin; // диапазон с учетом отступа
    ...
    final gridPaint = Paint()..color = _kBorder..strokeWidth = 0.5; // рисуем сетку
    ...
    for (var d = 0; d <= yDivs; d++) {
      ...
      final tp    = _tp(label, size: 9, color: _kTextDim); // текст
      tp.paint(canvas, Offset(padLeft - tp.width - 4, y - tp.height / 2)); // рисуем текст
    }

    // ── Метки оси X ───────────────────────────────────────────────────────────
    if (timestamps != null && timestamps!.isNotEmpty) { // если временных меток нет, то возвращаем
      const xDivs = 4; // количество делений по X
      for (var d = 0; d <= xDivs; d++) { // рисуем деления по X
        final idx = ((points.length - 1) * d / xDivs).round().clamp(0, points.length - 1); // индекс
        final x   = toX(idx); // координата X
        final lbl = _fmtX(idx); // метка
        if (lbl.isEmpty) continue; // если метка пустая, то пропускаем
        final tp = _tp(lbl, size: 9); // текст
        tp.paint(canvas, Offset((x - tp.width / 2).clamp(padLeft, size.width - padRight - tp.width), // координата X
            size.height - padBottom + 5)); // координата Y
      }
    }

    // ── Пунктирные пороговые линии ────────────────────────────────────────────
    void drawDash(double value, Color color, String lbl) { // функция для рисования пунктирных пороговых линий
      final y = toY(value); // координата Y
      if (y < padTop - 2 || y > size.height - padBottom + 2) return; // если координата Y меньше или больше, то пропускаем
      final p = Paint()..color = color.withOpacity(0.75)..strokeWidth = 1.0; // рисуем линию
      var x = padLeft; // координата X
      // Пунктир только до правого края области данных (не заходит в зону меток)
      while (x < size.width - padRight) { // рисуем линию
        canvas.drawLine(Offset(x, y), Offset((x + 5).clamp(0.0, size.width - padRight), y), p); // рисуем линию
        x += 9; // шаг
      }
      // Метка справа от области графика (в зоне padRight)
      final tp = _tp(lbl, size: 9, color: color); // текст
      final bgColor = isDark ? _kCard : Colors.white; // фон: тёмный/белый по теме
      final labelX = size.width - padRight + 4; // левый край метки — сразу за областью данных
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(labelX, y - tp.height / 2 - 2, tp.width + 6, tp.height + 4),
    ...
    if (warningMin != null) drawDash(warningMin!, _kWarning, 'W ${warningMin!.toStringAsFixed(1)}'); // рисуем пороговое значение для предупреждения
    ...
    // ── Заливка под линией ────────────────────────────────────────────────────
    final fillPath = Path()..moveTo(toX(0), size.height - padBottom); // рисуем заливку
    for (var i = 0; i < points.length; i++) {
      fillPath.lineTo(toX(i), toY(points[i])); // рисуем линию
    }
    fillPath.lineTo(toX(points.length - 1), size.height - padBottom); // рисуем линию
    fillPath.close(); // закрываем путь
    canvas.drawPath(fillPath, Paint()
      ..shader = LinearGradient( // рисуем градиент
        begin: Alignment.topCenter, // начало градиента
        end: Alignment.bottomCenter, // конец градиента
        colors: [lineColor.withOpacity(0.22), lineColor.withOpacity(0.01)], // цвета
      ).createShader(Rect.fromLTWH(padLeft, padTop, chartW, chartH)) // прямоугольник
      ..style = PaintingStyle.fill); // стиль заливки
    ...
    canvas.drawPath(linePath, Paint()
      ...
      ..strokeWidth = 2.0 // ширина линии
      ..style       = PaintingStyle.stroke // стиль линии
      ..strokeCap   = StrokeCap.round // закругление концов линии
      ..strokeJoin  = StrokeJoin.round); // соединение линий

    // Точки на концах (первая и последняя)
    for (final idx in [0, points.length - 1]) { // рисуем точки на концах (первая и последняя)
      canvas.drawCircle(Offset(toX(idx), toY(points[idx])), 3,
          Paint()..color = lineColor); // рисуем точку
    }
    ...
    if (hoverIndex != null && hoverIndex! < points.length) {
      ...
      // Вертикальная линия
      canvas.drawLine(Offset(hx, padTop), Offset(hx, size.height - padBottom), // рисуем линию
          Paint()..color = lineColor.withOpacity(0.30)..strokeWidth = 1.0);

      // Точка
      canvas.drawCircle(Offset(hx, hy), 6, Paint()..color = _kCard); // рисуем точку
      canvas.drawCircle(Offset(hx, hy), 4, Paint()..color = lineColor); // рисуем точку
      canvas.drawCircle(Offset(hx, hy), 4, Paint()
        ..color = lineColor.withOpacity(0.35) // цвет линии
        ..style = PaintingStyle.stroke // стиль линии
      ...
      final mainTp  = _tp(valLbl, size: 12, color: lineColor, fw: FontWeight.w700); // текст
      final timeTp  = timeLbl.isNotEmpty ? _tp(timeLbl, size: 10, color: _kTextDim) : null; 

      const tipPad = 7.0; // отступ
      final tipW = (timeTp != null ? timeTp.width.clamp(mainTp.width, 120.0) : mainTp.width) + tipPad * 2; // ширина толтипа
      final tipH = mainTp.height + (timeTp != null ? timeTp.height + 3 : 0) + tipPad * 2; // высота толтипа
      final tipX = (hx + 10 + tipW > size.width - padRight) ? hx - 10 - tipW : hx + 10; // координата X
      final tipY = (hy - tipH / 2).clamp(padTop, size.height - padBottom - tipH); // координата Y

      final tipRect = RRect.fromRectAndRadius( // рисуем прямоугольник
      ...
      canvas.drawRRect(tipRect, Paint() 
        ..color = lineColor.withOpacity(0.4) // цвет линии
        ..style = PaintingStyle.stroke
        ..strokeWidth = 0.8); // ширина линии

      mainTp.paint(canvas, Offset(tipX + tipPad, tipY + tipPad)); // рисуем текст
      timeTp?.paint(canvas, Offset(tipX + tipPad, tipY + tipPad + mainTp.height + 3));
    }
  }

  @override
  bool shouldRepaint(_ChartPainter old) { // функция для проверки, нужно ли перерисовывать график
    ...
    if (old.hoverIndex != hoverIndex) return true; // если индекс hover не равен, то перерисовываем
    if (old.isDark     != isDark)      return true; // если тема изменилась, перерисовываем
    if (old.warningMin != warningMin || old.warningMax != warningMax) return true; // если минимальное значение для предупреждения не равно, то перерисовываем
    if (old.alarmMin   != alarmMin   || old.alarmMax   != alarmMax)   return true; // если минимальное значение для тревоги не равно, то перерисовываем
    if (old.period     != period)      return true; // если период не равен, то перерисовываем
    if (old.points.length != points.length) return true; // если количество точек не равно, то перерисовываем
    if (old.timestamps?.length != timestamps?.length) return true; // если количество временных меток не равно, то перерисовываем
    for (var i = 0; i < points.length; i++) { // проверяем, нужно ли перерисовывать точки
      if (old.points[i] != points[i]) return true; // если значение точки не равно, то перерисовываем
    }
